                           ...
                           gnmi::Encoding encoding)
{
    ...
    case gnmi::JSON_IETF:
        ...
        try {
            ...
            cout << "Fail getting items from sysrepo: "
                 << exc.what() << endl;;
        ...
        for (auto it : json_vec) {
            ...
            if (!it.key.first.empty()) {
                cout << "putting list entries key in gNMI path";
    ...
    case gnmi::PROTO:
        cout << "Deviation from specification, Unsupported Yet";
        break;
                                      ...
                                      const SubscriptionList& request)
{
    ...
    for (int i = 0; i < request.subscription_size(); i++) {
        ...
        status = BuildSubsUpdate(updateList, sub.path(), gnmi_to_xpath(sub.path()),
                                 request.encoding());
        if (!status.ok()) {
            cout << "Fail building update for "
                 << gnmi_to_xpath(sub.path()) << endl;
            return status;
        }
    }
 ...
 */
Status
Subscribe::run(ServerContext *context,
               ServerReaderWriter<SubscribeResponse, SubscribeRequest> *stream)
{
    ...
    if (request.extension_size() > 0) {
        cout << "Extensions not implemented";
    ...
    default:
        cout << "Unknown subscription mode";
