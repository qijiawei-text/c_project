 ...
 *              < ePOE_STATUS_OK             - operation failed
 *---------------------------------------------------------------------*/
PoE_Status_e eDEVICE_COMMUNICATION_Get_15Bytes_CommProtocol_Reply(U8 bIC_Address, U8 byTxEcho, _OUT U8 byArrRxBuff[], _OUT POE_BOOL_e *pePOE_BOOL_Is_system_status)
{
 ...
 *              < ePOE_STATUS_OK                 - operation failed
 *---------------------------------------------------------------------*/
PoE_Status_e ReadAllRxBuffer(U8 bIC_Address, U8 byArrRxBuff[] , U16* pNumber_of_bytes_actually_read)
{
	/**********    Empty RX buffer
	- for RS232 UART buffer - wait for timeout
	- for I2C buffer - wait for 15 sequenced bytes 0 bytes */
