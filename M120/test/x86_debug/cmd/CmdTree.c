...
CMD_Node *LIST_First(CMD_List *pList)
{
    return pList->NODE_HEAD;
}

VOID LIST_Insert(CMD_List *pList,CMD_Node *pNode)
{
...
CMD_Node * LIST_Walk(CMD_List *pList, KEY_STRU stKey, int compare (void *, KEY_STRU))
{
...
int CMD_TreeCompare(void *p, KEY_STRU stkey)
{
...
VOID* CMD_FindTreeNode(CMD_List *pstlist, CHAR *pName)
{
    KEY_STRU stKey;
    stKey.pValue = pName;
...
ULONG CMD_AddTreeNode(CMD_List *pstlist, CMD_CmdInfo *pstCmd)
{
...
ULONG CMD_ISValueNode(CMD_CmdInfo *pstCmd)
{
...
VOID CMD_SetNodeType(CMD_CmdInfo *pstCmd, ULONG ulType, ULONG ulMode)
{
...
ULONG CMD_GetNodeType(ULONG ulSes, CMD_CmdInfo *pstCmd)
{
    ULONG ulRet = 0;
...
ULONG CMD_GetGroupType(ULONG ulSes, CMD_CmdInfo *pstCmd)
{
    if (pstCmd == NULL)
    {
        return FALSE;
...
ULONG CMD_BuildTree(CMD_TreeRecd* m_pastCmdTable)
{
    CMD_List     *pstlist;
    CMD_CmdInfo  *pstCmd;
...
ULONG CMD_IsCommandCanUse(ULONG ulSession, ULONG ulMode, ULONG ulLevel)
{
...
ULONG CMD_IsCommandCanDisplay(ULONG ulSession, ULONG ulMode, ULONG ulLevel)
{
    if (CmdCtrl[ulSession].UserLevel < ulLevel)
        return FALSE;
    if (ulMode == GLOB_MODE)
...
VOID CMD_DisplayPrompt(ULONG ulSes)
{
    ULONG ulLog2;
    ULONG ulLen = 0;
    CHAR  acFmt[128];
...
VOID CMD_InitUserMode(CMD_Ctrl *CmdCtrl)
{
...
VOID CMD_SetUserMode(ULONG ulSes, ULONG ulMode)
{
...
ULONG CMD_ClrScr(char *pcBuf, int Len, char **argv, int argc)
{
...
int CMD_HelpDisplay (char *pcBuf, int Len, char **argv, int argc)
{
...
ULONG CMD_ShowHistory(ULONG ulSes)
{
    ULONG ulHead;
    ULONG ulIndex;
...
ULONG CMD_History(char *pcBuf, int Len, char **argv, int argc)
{
...
unsigned long CMD_Reboot(char *pcBuf, int Len, char **argv, int argc)
{
    if (Len < 256 || argc < 0)
    {
        return CMD_ERROR;
...
ULONG CMD_Exit(char *pcBuf, int Len, char **argv, int argc)
{
    UINT ulSes = 0;
