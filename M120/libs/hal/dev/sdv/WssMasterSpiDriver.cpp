/*****************************************************************************************************
   ...
   FileName: WssMasterSpiDriver.cpp
   Author:
   Date:
   Version:
   Description:
...
/***FUNC-******************************************************************************************/
static int
_WssNis_IsReultErr(uint8_t *pucBuf)
{
    ST_WSS_NIS_RESP_S_HEAD_CMN *pstRespS = NULL;
    uint8_t ucRslt = 0;
    uint8_t ucRsltIndex = 0;
    char const *pacRslt[] = {
        "0 : No Error",
        "1 : Invalid Command",
    ...
    pstRespS = (ST_WSS_NIS_RESP_S_HEAD_CMN *)pucBuf;
    ucRslt = pstRespS->ucRslt;
    if ((pstRespS->ucMid == 0) && (pstRespS->ucLen == 2) && (pucBuf[3] == _WssNis_ChkSum(pucBuf, 3))) {
        if ((EN_WSS_NIS_ERR_NONE != ucRslt) && (EN_WSS_NIS_ERR_ENDOFDATA != ucRslt)) {
            ucRsltIndex = ucRslt > 13 ? 13 : ucRslt;
            LOG_WRITE(EN_LOG_ERROR, "WSS error: %d %s", ucRslt, pacRslt[ucRsltIndex]);
            return ucRslt;
        }
    } else {
        return OPLK_ERR;
...
/* Descrp : Parse the command from the frame                                                      */
...
/***FUNC-******************************************************************************************/
static int
_WssNis_ParseFrm(uint8_t *pucBuf, uint16_t *pusLen, uint8_t ucFrmType)
{
    uint16_t usLenFrm = 0;
    ST_WSS_NIS_RESP_S_HEAD_CMN *pstRespS = NULL;
    ST_WSS_NIS_RESP_L_HEAD_CMN *pstRespL = NULL;
    ...
    if (EN_WSS_NIS_FRM_TYPE_S == ucFrmType) {
        usLenFrm = pstRespS->ucLen + DEF_WSS_NIS_CMD_S_LEN_MID2LEN;
    } else {
        usLenFrm = ntohs(pstRespL->usLen) + DEF_WSS_NIS_CMD_L_LEN_MID2LEN_R;
    }

    if (DEF_WSS_NIS_BUF_LEN < usLenFrm) {
        /*LOG_WRITE(EN_LOG_ERROR, "Length error: %d", usLenFrm); */
        goto ERR_LABEL;
    }
...
#if 0
int
CWssMasterSpiDriver::__SendRecv(uint8_t *pucBufW, uint16_t usLenW,
                                uint8_t *pucBufR, uint16_t *pusLenR,
                                uint8_t ucFrmType, uint32_t uiTimeOut)
{
#if 0
    ...
    std::shared_ptr<CRspHandler> stpRsp = __RspHandler(uiTimeOut);
#define DEF_WSS_NIS_RECV_WAIT           (160)                               /* Unit: 0.25s for Nistica WSS                             */
    ...
    m_stpDrvPtr->ReadWrite(m_uiHwIdx, pucBufW, usLenW, nullptr, stpRsp);
                                ...
                                uint8_t ucFrmType, uint32_t uiTimeOut)
{
    ...
    uint16_t usTryW = 0;
    uint16_t usTryR = 0;
    std::shared_ptr<CRspHandler> stpEndPos = __RspHandler(uiTimeOut);;
...
#define DEF_WSS_NIS_RECV_WAIT           (160)                               /* Unit: 0.25s for Nistica WSS                             */
    ...
    for (usTryW = 0; usTryW < WSS_CMD_WR_RETRY_TMS; usTryW++) {
        ...
        bRet = m_stpDrvPtr->Write(m_uiHwIdx, pucBufW, usLenW);
        if (false == bRet) {
            LOG_WRITE(EN_LOG_ERROR, "ReadWrite error.");
            CDelay::Delay(5, MS);
            goto ERR_LABEL;
        ...
        for (usTryR = 0; usTryR < WSS_CMD_RD_RETRY_TMS; usTryR++) {
            ...
            m_stpDrvPtr->Read(m_uiHwIdx, aucBufR, stpEndPos);

            usLenR = stpEndPos->Len();
...
CWssMasterSpiDriver::__SendRecv(uint8_t *pucBufW, uint16_t usLenW,
                                uint8_t *pucBufR,
                                uint16_t *pusLenR, uint8_t ucFrmType, uint32_t usTimeoutMs)
{
    uint8_t aucBufR[DEF_WSS_NIS_BUF_LEN] = {0};
    uint8_t aucBufW[DEF_WSS_NIS_BUF_LEN] = {0};
    uint16_t usLenR = 0;
    uint16_t usTryR = 0;
    uint16_t usTryTime = 5;    /* ms */
    int iResult = OPLK_ERR;
    uint16_t usMaxLenR = 32;
    bool bRet = false;
    std::shared_ptr<CRspHandler> stpEndPos = __RspHandler(usTimeoutMs);

#define DEF_WSS_NIS_RECV_WAIT           (10)    /*(40)//(160) */

    if ((NULL == pucBufW) ||
        (NULL == pucBufR) ||
        (NULL == pusLenR) ||
        (0 == usLenW) ||
        (DEF_WSS_NIS_BUF_LEN < (usLenW + sizeof(ST_WSS_SPI_WR_HEADER)))) {
        LOG_WRITE(EN_LOG_ERROR, "Input parameter error");
        goto ERR_LABEL;
    }

    /*if(SDV_SpiMasterWriteWss(pucBufW, usLenW, usTimeoutMs) != OPLK_OK) */
    bRet = m_stpDrvPtr->Write(m_uiHwSubIdx, pucBufW, usLenW);
    if (false == bRet) {
        LOG_WRITE(EN_LOG_ERROR, "WSS spi write error");
        goto ERR_LABEL;
    }

    /* Make sure the data in the send buffer is 0, in order to avoid WSS receive random data and processing errors */
    memset(aucBufW, 0, sizeof(aucBufW));
    for (usTryR = 0; usTryR < DEF_WSS_NIS_RECV_WAIT; usTryR++) {
        stpEndPos->Reset();
        /*memset(aucBufR, 0, sizeof(aucBufR)); */
        /*if(SDV_SpiMasterReadWss(aucBufW, ((usMaxLenR * 2) > sizeof(aucBufW) ? sizeof(aucBufW) : (usMaxLenR * 2)), aucBufR, usMaxLenR, usTimeoutMs) != OPLK_OK) */
        memset(aucBufR, 0, sizeof(aucBufR));
        printf("stpEndPos->Len: %d\n", stpEndPos->Len());
        bRet = m_stpDrvPtr->Read(m_uiHwSubIdx, aucBufR, stpEndPos);
        if (false == bRet) {
            LOG_WRITE(EN_LOG_ERROR, "WSS spi read error, %d\n", usTryR);
            usleep(usTryTime * 1000);
            continue;
        }
        usLenR = stpEndPos->Len();
        printf("usLenR:%d\n", usLenR);
        LOG_WRITE(EN_LOG_NOTICE, "usLenR:%d", usLenR);
        /* Bad frame                                                        */
        if (OPLK_OK != _WssNis_ParseFrm(aucBufR, &usLenR, ucFrmType)) {
            LOG_WRITE(EN_LOG_ERROR, "WSS spi recv error, %d\n", usTryR);
            usleep(usTryTime * 1000);
            continue;
        }

        iResult = _WssNis_IsReultErr(aucBufR);
        if (OPLK_ERR != iResult) {
            LOG_WRITE(EN_LOG_NOTICE, "WSS _WssNis_IsReultErr,iResult: %d\n", iResult);
            usTryR = DEF_WSS_NIS_RECV_WAIT + usTryR;
            break;
        }
        ...
        /* Nistica WSS will reply "Inprogress" repeatly until other result  */
        if (true == _WssNis_IsInProc(aucBufR, usLenR, ucFrmType)) {
            LOG_WRITE(EN_LOG_NOTICE, "WSS spi in process, %d\n", usTryR);
            usleep(usTryTime * 1000);
            continue;
        }
    ...
    if (usTryR >= DEF_WSS_NIS_RECV_WAIT) {
        LOG_WRITE(EN_LOG_ERROR, "WSS spi Failed, %d", usTryR);
        goto ERR_LABEL;
    }
...
/***FUNC-******************************************************************************************/
int
CWssMasterSpiDriver::__WssNisCmd(uint8_t *pucBufW, uint16_t usLenW,
                                 uint8_t *pucBufR, uint16_t *pusLenR,
...
/***FUNC-******************************************************************************************/
int
CWssMasterSpiDriver::__WssNisCmdMultiW(ST_WSS_NIS_MULTI *pstMulti, uint16_t usCnt)
{
...
bool
CWssMasterSpiDriver::IsOnline(void)
{
...
bool
CWssMasterSpiDriver::GetMfg(CMfg &rstMfg)
{
#if 1
    uint16_t usLenR = 0;
    ...
    LOG_WRITE(EN_LOG_NOTICE, "CWssMasterSpiDriver::GetMfg +++");
    ...
    if (OPLK_OK != iRet) {
        LOG_WRITE(EN_LOG_ERROR, "CWssMasterSpiDriver::GetMfg ---");
        return false;
    }
    ...
    while ((pcTmp = strtok(NULL, pcDelim))) {
        ...
        LOG_WRITE(EN_LOG_NOTICE, "EN_WSS_NIS_VER_FW[%d] = %s", i, pcVerInfo[i]);
    }

    snprintf(rstMfg.acManufacturer, MFG_INFO_LEN_MAX, "%s", pcVerInfo[EN_WSS_NIS_VENDOR]);
    ...
    LOG_WRITE(EN_LOG_NOTICE, "acHwFuncRev %s", rstMfg.acHwFuncRev);
...
bool
CWssMasterSpiDriver::GetStatusData(CWssStatusData &rstData)
{
#if 0
    if (m_bCommFailDebug) {
        return false;
...
bool
CWssMasterSpiDriver::GetCfgData(CWssAllCfg &rstData)
{
    ...
    /* printf("Waveplan:\n"); */
    for (int i = 0; i < usChMax; ++i) {
        uint32_t uiChId = i + 1;
        uint32_t uiBand = ntohs(stNisVal.ausBand[uiChId]);
        uint32_t uiCenterFreq = ntohs(stNisVal.ausFreq[uiChId]);
...
#if 0
        if (i < 5) {
            printf("Id = %d, CenterFreq = %d, SliceCount = %d\n",
                   uiChId, rstData.astWavePlan[i].uiCenterFreq, rstData.astWavePlan[i].usSliceCount);
        }
#endif
    }
    ...
    for (int i = 0; i < usChMax; ++i) {
        ...
        if (i < 4) {
            /* printf("Id = %d, ComPort = %d, ExpPort = %d, Atten = %d\n", uiId, uiComPort, uiExpPort, stPortAttn[i].ucAttn); */
        }
...
#if 0
    printf("Cfg Up:\r\n");
    for (int i = 0; i < sizeof(rstData.astWssChCfgUp); ++i) {
        printf("%02x ", *((uint8_t *)rstData.astWssChCfgUp + i));
        if (((i + 1) % 32) == 0) {
            printf("\r\n");
        }
    }
    ...
    memset(stPortAttn, 0, sizeof(stPortAttn));
    memcpy(stPortAttn, stNisVal.ausPortAttn2 + 1, sizeof(stPortAttn));
...
bool
CWssMasterSpiDriver::SetCfg(uint32_t uiInstBegin, uint32_t uiInstEnd, CWssCfg *pstData, uint32_t uiSubIdx)
{
    return true;
}

bool
CWssMasterSpiDriver::SetOneChCfg(const CWssCfg &c_rstData, uint32_t uiSubIdx)
{
...
bool
CWssMasterSpiDriver::SetAllChCfg(uint32_t uiChCnt, CWssCfg *pstData, uint32_t uiSubIdx)
{
    ST_WSS_CH_INFO astCfg[WSS_CH_CNT_TOTAL];
    memset(&astCfg, 0, sizeof(astCfg));

#if 0
    printf("CWssMasterSpiDriver::SetAllChCfg Cfg +++ %d:\r\n", uiChCnt);
...
bool
CWssMasterSpiDriver::SetWavePlan(uint32_t uiChCnt, CWavePlan *pstWavePlan, uint32_t uiSubIdx)
{
#if 0
    int iChannelCnt = 1;
    double dSliceBand = 3.125;
    uint16_t usStartFreq = 61232; /* (191325 - 191000) / 3.125 = 104 */
    uint16_t usWidthCount = 10;  /* 12.5 / 3.125 = 4 */
    int rc = 0;
#endif
    ST_WSS_NIS_MULTI astWaveplan[WSS_CH_CNT_TOTAL];
    memset(&astWaveplan, 0, sizeof(astWaveplan));

    LOG_WRITE(EN_LOG_NOTICE, "WavePlan: ");
    ...
    for (uint32_t i = 0; i < uiChCnt; i++) {
        ...
        LOG_WRITE(EN_LOG_NOTICE, "%d %d %d\n",
                  astWaveplan[i].usCh,
                  astWaveplan[i].usFreq,
                  astWaveplan[i].usBand);
...
bool
CWssMasterSpiDriver::Reset(uint8_t ucResetType)
{
...
bool
CWssMasterSpiDriver::Upgrade(const char *pcFilePath)
{
...
#define DEF_WSS_NIS_RESET_WAIT          (10)                                /* Unit: 1s                                                */
    ...
    if (OPLK_OK != WssNisCmdUpgdR(EN_WSS_NIS_CMD_UPGD_COMMIT, 1000)) {
        LOG_WRITE(EN_LOG_ERROR, "Commit fw failure.");
        goto ABORT_LABEL;
    }
...
CWssMasterSpiDriver::__RspHandler(uint32_t uiLen, uint32_t uiTimeOut)
{
    std::shared_ptr<CEndToken> stpEndToken = std::static_pointer_cast<CEndToken>(std::make_shared<CWssEndToken>());
