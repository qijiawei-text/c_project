...
public:
    static void
    SetDevPtr(const std::shared_ptr<COscDriver> &c_rstpDrvPtr, uint32_t uiDevIdx)
    {
    ...
    static bool SetLaserEnable(uint32_t uiLen, const uint8_t *cv_pucBuf);
    static bool Reset(uint32_t uiLen, const uint8_t *cv_pucBuf);

private:
    static std::map<uint32_t, std::shared_ptr<COscDriver> > ms_staDrvPtrs;
