...
public:
    static void
    SetDevPtr(const std::shared_ptr<CExtendDriver> &c_rstpDrvPtr, uint32_t uiDevIdx)
    {
    ...
    static bool SetSwitchPos(uint32_t uiLen, const uint8_t *cv_pucBuf);
    static bool SetSfp(uint32_t uiLen, const uint8_t *cv_pucBuf);
    static bool SetVoaAttn(uint32_t uiLen, const uint8_t *cv_pucBuf);
...
private:
    static std::map<uint32_t, std::shared_ptr<CExtendDriver> > ms_staDrvPtrs;
