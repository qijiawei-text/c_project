...
public:
    static void
    SetDevPtr(const std::shared_ptr<CWssDriver> &c_rstpDrvPtr, uint32_t uiDevIdx)
    {
    ...
    static bool SetCfg(uint32_t uiLen, const uint8_t *cv_pucBuf);
...
private:
    static std::map<uint32_t, std::shared_ptr<CWssDriver> > ms_staDrvPtrs;
